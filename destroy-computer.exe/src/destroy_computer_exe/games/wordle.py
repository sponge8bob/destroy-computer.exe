...
from destroy_computer_exe.ui.theme import BOARD_STYLES, KEY_STYLES, console
...
def _load_words(filename: str) -> list[str]:
    text = files("destroy_computer_exe").joinpath("data").joinpath(filename).read_text(encoding="utf-8")
...
def _render(guesses: list[tuple[str, list[str]]], message: str = "") -> None:
    console.clear()
    console.rule("[bold white]DESTROY-COMPUTER.EXE[/bold white]", style="dim white")
    console.print()
...
def _share_text(guesses: list[tuple[str, list[str]]], answer: str) -> str:
    ...
    rows = [f"destroy-computer.exe wordle  {today}  {score_str}", ""]
